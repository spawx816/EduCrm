                                ...
                                {suggestions.enrollmentSuggestions.map((enrollSeg: any) => (
                                    <div key={enrollSeg.enrollmentId} className="space-y-2 border-l-2 border-indigo-500/20 pl-4">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            ...
                                            {enrollSeg.suggestedModule && (
                                                <button
                                                    type="button"
                                                    onClick={() => handleToggleSuggestedItem('MOD', enrollSeg.suggestedModule, enrollSeg)}
                                                    className={`px-2 py-1 rounded-lg border text-[9px] flex items-center space-x-1 transition-all ${selectedItems.some(i => i.itemId === 'MOD-' + enrollSeg.suggestedModule.id && i.enrollmentId === enrollSeg.enrollmentId)
                                                        ? 'bg-emerald-600 text-white border-emerald-500'
                                                        : 'bg-slate-950 border-slate-800 text-emerald-400 hover:border-emerald-500/50'
                                                        }`}>
                                                    <Check className="w-2 h-2" />
                                                    <span className="font-bold">{enrollSeg.suggestedModule.name}: RD${parseFloat(enrollSeg.suggestedModule.price).toLocaleString()}</span>
                                                </button>
                                            )}
                                            {enrollSeg.addons?.map((addon: any) => (
                                                <button
                                                    ...
                                                    className={`px-2 py-1 rounded-lg border text-[9px] flex items-center space-x-1 transition-all ${selectedItems.some(i => i.itemId === addon.id)
                                                        ? 'bg-indigo-600 text-white border-indigo-500'
